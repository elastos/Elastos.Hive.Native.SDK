...
static ssize_t onedrive_file_lseek(HiveFile *base, ssize_t offset, int whence)
{
    ...
    vlogD("OneDriveFile: Calling onedrive_file_lseek().");
    ...
    default:
        assert(0);
        vlogE("OneDriveFile: file lseek failure: unsupported whence parameter.");
        return HIVE_GENERAL_ERROR(HIVEERR_NOT_SUPPORTED);
    }

    rc = lseek(file->fd, offset, whence_sys);
    if (rc < 0) {
        vlogE("OneDriveFile: file lseek failure: failed to call lseek().");
...
static ssize_t onedrive_file_read(HiveFile *base, char *buf, size_t bufsz)
{
    ...
    vlogD("OneDriveFile: Calling onedrive_file_read().");

    rc = read(file->fd, buf, bufsz);
    if (rc < 0) {
        vlogE("OneDriveFile: failed to read: failed to call read().");
        return HIVE_SYS_ERROR(errno);
    }
...
static ssize_t onedrive_file_write(HiveFile *base, const char *buf, size_t bufsz)
{
    ...
    vlogD("OneDriveFile: Calling onedrive_file_write().");
                                 ...
                                 char *upload_url, size_t upload_url_len)
{
    ...
    vlogD("OneDriveFile: Calling create_upload_session().");
    ...
    if (rc) {
        vlogE("OneDriveFile: failed to create upload session: failed to create http client instance.");
        return HIVE_HTTPC_ERROR(rc);
    }

    rc = http_client_get_response_code(httpc, &resp_code);
    if (rc) {
        vlogE("OneDriveFile: failed to create upload session: failed to get http response code.");
        return HIVE_HTTPC_ERROR(rc);
    }

    if (resp_code == HttpStatus_Unauthorized) {
        vlogE("OneDriveFile: failed to create upload session: access token expired.");
        oauth_token_set_expired(file->token);
        return HIVE_GENERAL_ERROR(HIVEERR_TRY_AGAIN);
    }

    if (resp_code != HttpStatus_OK) {
        vlogE("OneDriveFile: failed to create upload session: error from http response (%d).", resp_code);
        return HIVE_HTTP_STATUS_ERROR(resp_code);
    }

    p = http_client_move_response_body(httpc, NULL);
    if (!p) {
        vlogE("OneDriveFile: failed to create upload session: failed to get http response body.");
    ...
    if (!session) {
        vlogE("OneDriveFile: failed to create upload session: failed to parse json object from response.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }
    ...
    if (!upload_url_json || !cJSON_IsString(upload_url_json) ||
        ...
        vlogE("OneDriveFile: failed to create upload session: failed to create json object.");
    ...
    if (rc < 0 || rc >= upload_url_len) {
        vlogE("OneDriveFile: failed to create upload session: upload URL too long.");
                                                ...
                                                void *userdata)
{
    ...
    vlogD("OneDriveFile: Calling upload_to_session_request_body_cb().");
    ...
    if (nrd < 0) {
        vlogE("OneDriveFile: failed to upload file: failed to call read().");
                             ...
                             const char *upload_url)
{
    ...
    vlogD("OneDriveFile: Calling upload_to_session().");
    ...
    while (ul_sz) {
        ...
        rc = http_client_request(httpc);
        if (rc) {
            vlogE("OneDriveFile: failed to upload file: failed to perform http request (%d).", rc);
            return HIVE_HTTPC_ERROR(rc);
        }
        ...
        rc = http_client_get_response_code(httpc, &resp_code);
        if (rc) {
            vlogE("OneDriveFile: failed to upload file: failed to get http response code.");
            return HIVE_HTTPC_ERROR(rc);
        }
        ...
        if ((ul_sz && resp_code != HttpStatus_Accepted) ||
            ...
            vlogE("OneDriveFile: failed to upload file: error from http response (%d).", resp_code);
            return HIVE_HTTP_STATUS_ERROR(resp_code);
        }
    }
...
static int upload_file(OneDriveFile *file)
{
    ...
    vlogD("OneDriveFile: Calling upload_file().");

    rc = oauth_token_check_expire(file->token);
    if (rc < 0) {
        vlogE("OneDriveFile: failed to upload file: checking access token expired error.");
        return rc;
    }

    httpc = http_client_new();
    if (!httpc) {
        vlogE("OneDriveFile: failed to upload file: failed to create http client instance.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }

    rc = create_upload_session(file, httpc, upload_url, sizeof(upload_url));
    if (rc < 0) {
        vlogE("OneDriveFile: failed to upload file: failed to create upload session.");
    ...
    http_client_close(httpc);
    if (rc < 0) {
        vlogE("OneDriveFile: failed to upload file: failed to upload to session.");
        return rc;
    }
                         ...
                         char *dl_url, size_t dl_url_len)
{
    ...
    vlogD("OneDriveFile: Calling get_file_stat().");

    rc = oauth_token_check_expire(token);
    if (rc < 0) {
        vlogE("OneDriveFile: failed to get file status: checking access token expired error.");
        return rc;
    }

    httpc = http_client_new();
    if (!httpc) {
        vlogE("OneDriveFile: failed to get file status: failed to create http client instance.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }
    ...
    if (rc) {
        ...
        vlogE("OneDriveFile: failed to get file status: failed to perform http request (%d).", rc);
        goto error_exit;
    }
    ...
    if (rc) {
        rc = HIVE_HTTPC_ERROR(rc);
        vlogE("OneDriveFile: failed to get file status: failed to get http response code (%d).", rc);
        goto error_exit;
    }

    if (resp_code == HttpStatus_Unauthorized) {
        vlogE("OneDriveFile: failed to get file status: access token expired.");
    ...
    if (resp_code != HttpStatus_OK) {
        vlogE("OneDriveFile: failed to get file status: error from http response (%d).", resp_code);
    ...
    if (!p) {
        vlogE("OneDriveFile: failed to get file status: failed to get http response body.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }
    ...
    if (!fstat) {
        vlogE("OneDriveFile: failed to get file status: bad json format for response.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }

    if (!cJSON_GetObjectItemCaseSensitive(fstat, "file")) {
        vlogE("OneDriveFile: failed to get file status: missing file json object for response.");
        cJSON_Delete(fstat);
        return HIVE_GENERAL_ERROR(HIVEERR_BAD_JSON_FORMAT);
    }
    ...
    if (!download_url_json || !download_url_json->string ||
        ...
        vlogE("OneDriveFile: failed to get file status: missing @microsoft.graph.downloadUrl json object for response.");
        cJSON_Delete(fstat);
        return HIVE_GENERAL_ERROR(HIVEERR_BAD_JSON_FORMAT);
    }
    ...
    if (!ctag_json || !ctag_json->string || !*ctag_json->valuestring ||
        ...
        vlogE("OneDriveFile: failed to get file status: missing cTag json object for response.");
        cJSON_Delete(fstat);
        return HIVE_GENERAL_ERROR(HIVEERR_BAD_JSON_FORMAT);
    }
                                     ...
                                     size_t nitems, void *userdata)
{
    ...
    vlogD("OneDriveFile: Calling response_body_callback().");

    nwr = write(fd, buffer, total_sz);
    if (nwr < 0) {
        vlogE("OneDriveFile: failed to get file chunk: calling write() failure.");
        return 0;
    }
...
static int download_file(int fd, const char *download_url)
{
    ...
    vlogD("OneDriveFile: Calling download_file().");

    httpc = http_client_new();
    if (!httpc) {
        vlogE("OneDriveFile: failed to download from onedrive: failed to create http client.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }
    ...
    if (rc) {
        rc = HIVE_HTTPC_ERROR(rc);
        vlogE("OneDriveFile: failed to download from onedrive: failed to perform http request (%d).", rc);
        goto error_exit;
    }
    ...
    http_client_close(httpc);
    if (rc) {
        vlogE("OneDriveFile: failed to download from onedrive: failed to get http response code.");
        return HIVE_HTTPC_ERROR(rc);
    }

    if (resp_code != HttpStatus_OK) {
        vlogE("OneDriveFile: failed to download from onedrive: error from http response (%d).", resp_code);
        return HIVE_HTTP_STATUS_ERROR(resp_code);
    }
...
static int onedrive_file_commit(HiveFile *base)
{
    ...
    vlogD("OneDriveFile: Calling onedrive_file_commit().");
    ...
    rc = upload_file(file);
    if (rc < 0) {
        vlogE("OneDriveFile: failed to commit temporary file (%d).", rc);
        return rc;
    }
...
static int mkstemp(char *template)
{
    ...
    vlogD("OneDriveFile: Calling mkstemp().");

    err = _mktemp_s(template, strlen(template) + 1);
    if (err) {
        vlogE("OneDriveFile: failed to call _mktemp_s().");
        errno = err;
        return -1;
    }
                       ...
                       int flags, const char *tmp_template, HiveFile **file)
{
    ...
    vlogD("OneDriveFile: Calling onedrive_file_open().");
    ...
    if (rc < 0 && rc != HIVE_HTTP_STATUS_ERROR(HttpStatus_NotFound)) {
        vlogE("OneDriveFile: failed to open file: get file status failure (%d).", rc);
        return rc;
    }
    ...
    if (HIVE_F_IS_EQ(flags, HIVE_F_RDONLY)) {
        if (!file_exists) {
            vlogE("OneDriveFile: failed to open file: set readonly flag while file does not exist.");
            return HIVE_GENERAL_ERROR(HIVEERR_INVALID_ARGS);
        }
    } else if (HIVE_F_IS_SET(flags, HIVE_F_CREAT | HIVE_F_EXCL) && file_exists) {
        vlogE("OneDriveFile: failed to open file: set EXCL flag while file exists.");
        return HIVE_GENERAL_ERROR(HIVEERR_INVALID_ARGS);
    } else if (!HIVE_F_IS_SET(flags, HIVE_F_CREAT) && !file_exists) {
        vlogE("OneDriveFile: failed to open file: set no CREAT flag while file does not exist.");
        return HIVE_GENERAL_ERROR(HIVEERR_INVALID_ARGS);
    }

    tmp = rc_zalloc(sizeof(OneDriveFile), onedrive_file_destructor);
    if (!tmp) {
        vlogE("OneDriveFile: failed to open file: out of memory.");
        return HIVE_GENERAL_ERROR(HIVEERR_OUT_OF_MEMORY);
    }
    ...
    if (tmp->fd < 0) {
        vlogE("OneDriveFile: failed to open file: failed to call mkstemp().");
        deref(tmp);
        return HIVE_SYS_ERROR(errno);
    }
    ...
    if (file_exists && !HIVE_F_IS_SET(flags, HIVE_F_TRUNC)) {
        ...
        if (rc < 0) {
            vlogE("OneDriveFile: failed to open file: failed to download from onedrive.");
            unlink(tmp->tmp_path);
            deref(tmp);
            return rc;
        }
