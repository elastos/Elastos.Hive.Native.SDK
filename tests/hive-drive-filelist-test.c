...
static void test_hive_drive_filelist_without_drive(void)
{
    int rc;
    char *result;

    rc = hive_drive_list_files(NULL, global_config.files.file_path, &result);
    CU_ASSERT_EQUAL(rc, -1);

    return;
}

static void test_hive_drive_filelist_without_path(void)
{
    int rc;
    char *result;

    rc = hive_drive_list_files(drive, NULL, &result);
    CU_ASSERT_EQUAL(rc, -1);
...
static void test_hive_drive_filestat_without_result(void)
{
    ...
    rc = hive_drive_list_files(drive, global_config.files.file_path, NULL);
    CU_ASSERT_EQUAL(rc, -1);

    return;
}

static void test_hive_drive_filelist(void)
{
    int rc;
    char* result;

    rc = hive_drive_list_files(drive, global_config.files.file_path, &result);
    CU_ASSERT_EQUAL(rc, 0);

    return;
}

static int hive_drive_filelist_test_suite_init(void)
{
    int rc;
...
static int hive_drive_filelist_test_suite_cleanup(void)
{
    int rc;
...
static CU_TestInfo cases[] = {
    {   "test_hive_drive_filelist_without_drive",     test_hive_drive_filelist_without_drive  },
    {   "test_hive_drive_filelist_without_path",      test_hive_drive_filelist_without_path   },
    {   "test_hive_drive_filelist",                   test_hive_drive_filelist                },
    {   NULL,                                         NULL                                    }
};

static CU_SuiteInfo suite[] = {
    {   "hive drive filelist test", hive_drive_filelist_test_suite_init, hive_drive_filelist_test_suite_cleanup, NULL, NULL, cases },
