...
static void test_hive_drive_getinfo_without_client(void)
{
    int rc;
    char *result;

    rc = hive_drive_get_info(NULL, &result);
    CU_ASSERT_EQUAL(rc, -1);

    return;
}

static void test_hive_drive_getinfo_without_result(void)
{
    int rc;

    rc = hive_drive_get_info(drive, NULL);
    CU_ASSERT_EQUAL(rc, -1);

    return;
}

static void test_hive_drive_getinfo(void)
{
    int rc;
    char *result;

    rc = hive_drive_get_info(dirve, &result);
...
static int hive_drive_getinfo_test_suite_init(void)
{
    int rc;
...
static int hive_drive_getinfo_test_suite_cleanup(void)
{
    int rc;
...
static CU_TestInfo cases[] = {
    {   "test_hive_drive_getinfo_without_drive",      test_hive_drive_getinfo_without_drive   },
    {   "test_hive_drive_getinfo_without_result",     test_hive_drive_getinfo_without_result  },
    {   "test_hive_drive_getinfo",                    test_hive_drive_getinfo                 },
    {   NULL,                                         NULL                                     }
};drive

static CU_SuiteInfo suite[] = {
    {   "hive drive getinfo test", hive_drive_getinfo_test_suite_init, hive_drive_getinfo_test_suite_cleanup, NULL, NULL, cases },
